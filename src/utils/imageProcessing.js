/**
 * Image processing utilities for CleanCut Studio
 ...
 */
export const applyMaskToImage = async (originalCanvas, mask, width, height, options = {}) => {
...
/**
 * Apply custom background to processed image
 * @param {string} processedImageUrl - Transparent image data URL
 * @param {Object} bgOptions - Background options
 * @returns {Promise<string>} Data URL with background
 */
export const applyBackground = async (processedImageUrl, bgOptions) => {
  const { type, color, gradient, blur } = bgOptions;
    ...
    img.onload = () => {
      ...
      switch (type) {
        ...
        case 'gradient':
          ...
          grd.addColorStop(0, gradient?.start || '#667eea');
          grd.addColorStop(1, gradient?.end || '#764ba2');
          ctx.fillStyle = grd;
          ctx.fillRect(0, 0, canvas.width, canvas.height);
          break;
 ...
 */
export const downloadCanvasAsImage = (canvas, filename, format = 'png') => {
